...
public:
	...
	game_object(model* m, IRenderShader* s, float4x4& w) : _model(m), _world(w), _shader(s), mats(m->Meshes().size())
	{
	}

	game_object(model* m, IRenderShader* s, float3 _pos = float3(), float3 _rot = float3(), vector<texture2d*> texs = vector<texture2d*>(), float3 _scl = float3(1,1,1))
	...
	virtual void draw_with_shader(ComPtr<ID3D11DeviceContext1> context, IRenderShader* shad);
